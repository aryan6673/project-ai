...
6. Also, don't forget to show female nature."""
    ...
    def create_widgets(self):
        # Configure grid layout: header, chat display, typing indicator, and input area
        ...
        # Initial messages without emojis
    ...
    def show_response_animated(self, response_text):
        ...
        # Insert assistant prefix in blue
        self.chat_frame.configure(state="normal")
        self.chat_frame.insert(tk.END, "• Ana: ", "assistant_prefix")
        self.chat_frame.configure(state="disabled")
        self.chat_frame.see(tk.END)
        
        # Check if response contains code blocks (text wrapped in triple backticks)
        if "```" not in response_text:
            # Animate normal assistant text
            ...
            # Split text on triple backticks; even indices = normal text, odd = code block
            ...
            def animate_segments(seg_index=0):
                if seg_index < len(segments):
                    ...
                    def animate_chars_in_segment(char_index=0):
                        if char_index < len(seg_text):
                            self.chat_frame.configure(state="normal")
                            self.chat_frame.insert(tk.END, seg_text[char_index], current_tag)
                            self.chat_frame.configure(state="disabled")
                            self.chat_frame.see(tk.END)
                            self.after(10, lambda: animate_chars_in_segment(char_index+1))
                        else:
                            # Optionally add a newline after a code block segment for clarity
                            if seg_index % 2 == 1:
                                self.chat_frame.configure(state="normal")
                                self.chat_frame.insert(tk.END, "\n", current_tag)
                                self.chat_frame.configure(state="disabled")
